...
const LegalHelp: React.FC = () => {
  ...
  useEffect(() => {
    const getPage = async () => {
      try {
        ...
        setFormData({ legalhelp: data[0]?.legalhelp || "" }); // Pre-fill form with existing data
      } catch (err) {
  ...
  const handleChange = (value: string) => {
    setFormData((prev) => ({ ...prev, legalhelp: value }));
  };
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      <Header profileImage={profileImage} userName={adminName} />
        <div className="p-6">
          <h1 className="text-3xl font-bold mb-6">Legal Help</h1>
